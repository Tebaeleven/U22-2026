...
export class BlockEditorController {
  ...
  readonly interactionOverrides: Partial<InteractionConfig> = {
    ...
    onNest: (container, zone) =>
      this.handleNestChange(container, zone, true),
    onUnnest: (container, zone) =>
      this.handleNestChange(container, zone, false),
  ...
  private rebuildScene(workspace: Workspace, containers: Container[]): void {
    ...
    seedInitialCBlockNest(scene.repeat1, [
      scene.moveInRepeat,
      scene.turnInRepeat,
    ])
  ...
  private syncRenderOrder(): boolean {
    return this.applyRenderOrder(
      sortContainersForNestedRender(this.containers)
    )
  }
  ...
  private applyRenderOrder(nextContainers: Container[]): boolean {
    ...
    const blocksChanged =
      ...
      nextBlocks.some(
        (block, index) => block !== this.snapshot.blocks[index]
      )
